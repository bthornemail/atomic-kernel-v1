...
echo "[1/4] conformance tests"
bash "$ROOT/conformance/run-tests.sh" >/dev/null

echo "[2/4] atomic-kernel gate"
bash "$ROOT/scripts/atomic-kernel-gate.sh" >/dev/null

echo "[3/4] replay-hash files present and valid"
for f in \
...
echo "[4/5] package metadata sanity"
python3 - <<PY >/dev/null
...
echo "[5/5] downstream public import boundary"
