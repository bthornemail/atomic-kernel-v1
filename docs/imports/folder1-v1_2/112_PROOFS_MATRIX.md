...
## Chirality Integration Update (v1.3)

Decision: keep the `112` structure unchanged.

- `8 invariant questions × 7 algorithms × 2 proof forms = 112`
- Chirality is integrated as a required sub-algorithm of A5.

A5 is now defined as:
...
`S -> (S0, S1)` and `chirality = f(kernel_state, tick)` then `ordered = (S_chirality, S_other)`.

A5 must prove:

- partition correctness
- chirality determinism
- label invariance
- replay invariance
...
5. **Partition & Chirality (Fano Selection) Algorithm**  
   Produces balanced partitions and deterministic orientation derived from canonical state.
...
| Fano Path | Partition topology is deterministic and chirality ordering is kernel-derived per tick. | Any hardcoded subset ordering or non-kernel orientation is invalid. |
...
| Fano Path | Refinement shifts are deterministic by tick with kernel-derived chirality. | Arbitrary subset rotation or label-derived first-set selection is invalid. |
...
| Fano Path | A5 controls partition+orientation only; it does not parse stream classes. | Using A5 as parser authority violates control-plane law. |
...
| Fano Path | Chirality selects orientation deterministically while preserving canonical truth boundary. | Projection/UI order changing A5 winner is invalid. |
...
| Fano Path | User/agent may influence advisory horizon only; canonical orientation remains kernel-derived. | Escape/local scope overriding chirality derivation is invalid. |
...
| Fano Path | Shared selection context is inspectable and chirality source is derivable from canonical state. | Opaque ordering or hidden chirality source is invalid. |
...
| Fano Path | Branch selection preserves origin and chirality derivation provenance. | Branch merge/selection without chirality consistency is invalid. |
...
| Fano Path | New partition schemes remain valid only with kernel-derived chirality. | Extension introducing implicit ordering is invalid. |
